...
def evaluate_route_cost(route):
    ...
    if distribution_judgment(distribution_result):
        config = load_config()
        env = cold_chain_env.ColdChainEnv(config)
        for item in distribution_result:
            ...
            except cold_chain_env.DistributionException as e:
                causer: cold_chain_env.VehicleFulledException = e.causer
            ...
            for i in range(len(action_distribution_orders)):
                ...
                except cold_chain_env.UnloadException as e:
                    causer: cold_chain_env.ItemNotEnoughException = e.causer
        ...
        return need_time_total, -all_satisfaction / len(ORDERS), all_cost
    else:
