...
def run(maxgen=100):
    ...
    init_chromsome = [i + 1 for i in range(lens_chrom)]
    prophetPop_ready = []

    for _ in range(50):
        random.shuffle(init_chromsome)
        prophetPop_ready.append(init_chromsome)
    random.shuffle(prophetPop_ready)
    Fields = np.vstack([
        np.ones(lens_chrom, dtype=int),  # 下界（全0）
        np.full(lens_chrom, lens_chrom, dtype=int),  # 上界（num-1）
        np.ones(lens_chrom, dtype=int)
    ])
    prophetPop = ea.Population(Encoding='P', NIND=50, Chrom=np.array(prophetPop_ready), Field=Fields)
