...
class HybridSelection:
    def __init__(self, p_m=0.2, alpha=1.0, warmup=0.2, warm_mid=0.4, maxgens=100):
        self.p_m = p_m
        self.alpha = alpha
        self.m_selector = MSelection(alpha=alpha)
...
def run(maxgen=100):
    ...
    algorithm = moea_NSGA2_templetss(
        ...
        selectStyle=HybridSelection(0.5, 1.0, 0.1, 0.4, maxgens=maxgen)  # 使用混合选择算子
    )
    res = ea.optimize(
        algorithm,
        verbose=True,
